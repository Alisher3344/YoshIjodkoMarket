...
import { ShoppingCart, Check, ArrowLeft, Tag, User, Package, Share2, MapPin, Phone, School } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">{lang === 'uz' ? 'Mahsulot topilmadi' : 'Товар не найден'}</p>
          <Link to="/products" className="text-[#ffffff] font-bold">{t('back')}</Link>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid lg:grid-cols-2 gap-10 bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col">
            {/* Category */}
            <span className="text-sm font-bold text-[#ffffff] uppercase tracking-wider mb-3">{product.category}</span>

            {/* Name */}
            <h1 className="text-3xl font-black text-[#4c1d95] mb-4 leading-tight font-serif">{name}</h1>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              <div className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl">
                ...
                <div>
                  <p className="text-xs text-gray-400">{t('prod_by').replace(':', '')}</p>
                  <p className="text-sm font-bold text-gray-700">{product.author}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl">
                <Tag className="w-4 h-4 text-[#4c1d95]" />
                <div>
                  <p className="text-xs text-gray-400">{lang === 'uz' ? 'Sinf' : 'Класс'}</p>
                  <p className="text-sm font-bold text-gray-700">{classLabel}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl">
                <Package className="w-4 h-4 text-[#4c1d95]" />
                <div>
                  <p className="text-xs text-gray-400">{lang === 'uz' ? 'Omborda' : 'В наличии'}</p>
                  <p className={`text-sm font-bold ${product.stock > 0 ? 'text-green-600' : 'text-red-500'}`}>
                    {product.stock > 0 ? `${product.stock} dona` : t('prod_out')}
                  </p>
                </div>
              </div>
              {product.school && (
                <div className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs text-gray-400">{lang === 'uz' ? 'Maktab' : 'Школа'}</p>
                    <p className="text-sm font-bold text-gray-700">{product.school}</p>
                  </div>
                </div>
              )}
              {product.district && (
                <div className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs text-gray-400">{lang === 'uz' ? 'Tuman / Shahar' : 'Район / Город'}</p>
                    <p className="text-sm font-bold text-gray-700">{product.district}</p>
                  </div>
                </div>
              )}
              {product.phone && (
                <div className="flex items-center gap-2 bg-[#ede9fe] px-4 py-2 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs text-[#4c1d95]/60">{lang === 'uz' ? 'Bog\'lanish' : 'Контакт'}</p>
                    <a href={`tel:${product.phone}`} className="text-sm font-bold text-[#4c1d95] hover:underline">
                      {product.phone}
                    </a>
                  </div>
                </div>
            ...
            {/* Description */}
            <div className="mb-6">
              <h3 className="font-black text-[#4c1d95] mb-2">{t('prod_description')}</h3>
            ...
            <div className="mb-6 p-5 bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl">
              ...
              <p className="text-4xl font-black text-[#4c1d95]">
                {formatPrice(product.price)} <span className="text-lg text-gray-400">{t('sum')}</span>
              </p>
            </div>
            ...
            <div className="flex gap-3 mt-auto">
              ...
              >
                {inCart ? <><Check className="w-5 h-5" /> {t('prod_in_cart')}</> : <><ShoppingCart className="w-5 h-5" /> {t('prod_add_cart')}</>}
              </button>
            ...
            {inCart && (
              <Link to="/cart"
                className="mt-3 text-center text-[#ffffff] font-bold text-sm hover:underline">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {related.map(p => (
                <div key={p.id} className="animate-fadeInUp">
                  <Link to={`/products/${p.id}`} className="block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100 group">
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-[#ffffff] font-black mt-2">{formatPrice(p.price)} {t('sum')}</p>
                    </div>
                  </Link>
                </div>
              ))}
