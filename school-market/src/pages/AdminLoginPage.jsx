...
export default function AdminLoginPage() {
  ...
  const handleLogin = async () => {
    ...
    const isAdmin = uname === 'admin' && upass === 'admin2008#'

    // 2. Check users from store (same browser)
    const userMatch = users.find(
      u => u.username.trim() === uname && u.password.trim() === upass && u.active !== false
    )

    // 3. Check localStorage directly (cross-tab)
    let storageMatch = null
    try {
      const raw = localStorage.getItem('users-storage')
      if (raw) {
        const all = JSON.parse(raw)?.state?.users || []
        storageMatch = all.find(
          u => u.username?.trim() === uname && u.password?.trim() === upass && u.active !== false
        )
      }
    } catch (e) {}

    const found = isAdmin || userMatch || storageMatch

    if (found) {
      login('admin', 'admin2008#') // set isLoggedIn = true
      const name = userMatch?.name || storageMatch?.name || 'Admin'
        ...
        <div className="bg-white rounded-3xl p-8 shadow-2xl animate-fadeInUp">
          ...
          <div className="bg-[#ede9fe] border border-[#4c1d95]/20 rounded-xl p-3 mb-6">
            <p className="text-xs text-[#4c1d95] font-semibold text-center">
              {lang === 'uz' ? 'Superadmin: admin / admin2008#' : 'Суперадмин: admin / admin2008#'}
            </p>
          </div>
