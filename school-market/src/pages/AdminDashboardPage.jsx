...
import {
  ...
  Eye, EyeOff, Shield, UserCheck, UserX, Key
} from 'lucide-react'
...
export default function AdminDashboardPage() {
  ...
  const { orders, updateStatus, getStats } = useOrderStore()
  const { users, addUser, updateUser, deleteUser, toggleActive } = useUsersStore()
  const { customOrders, updateCustomStatus } = useCustomOrderStore()
  ...
  const handleSave = () => {
    ...
    if (editId) { updateProduct(editId, data); toast.success('Yangilandi!') }
    else { addProduct(data); toast.success("Qo'shildi!") }
    setShowForm(false); setEditId(null)
  }

  const handleDelete = (id) => { deleteProduct(id); setConfirmDel(null); toast.success("O'chirildi!") }
        ...
        {tab === 'products' && (
          <div>
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm text-gray-500">{products.length} {lang==='uz'?'ta mahsulot':'товаров'}</p>
