...
import { ArrowRight, Star, Sparkles, TrendingUp, Shield, Zap, Palette, MessageCircle } from 'lucide-react'
...
export default function HomePage() {
  ...
  const features = [
    {icon:<Zap className="w-6 h-6"/>,uz:"Tez yetkazish",ru:'Быстрая доставка',duz:"1-3 kun ichida",dru:"За 1-3 дня"},
    {icon:<Shield className="w-6 h-6"/>,uz:"Xavfsiz to'lov",ru:'Безопасная оплата',duz:"Click, Payme, Uzum",dru:"Click, Payme, Uzum"},
    {icon:<Palette className="w-6 h-6"/>,uz:"Asl ijod",ru:'Ручная работа',duz:"Har biri noyob",dru:"Каждая — уникальна"},
    {icon:<MessageCircle className="w-6 h-6"/>,uz:"24/7 Yordam",ru:'Поддержка 24/7',duz:"Telegram orqali",dru:"Через Telegram"},
  ]
      ...
      <section className="min-h-screen hero-pattern flex items-center relative overflow-hidden pt-20">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-20 grid lg:grid-cols-2 gap-16 items-center">
          <div className="text-white">
            <div className="flex items-center gap-6 mb-6 -mt-16">
              <img src="/logo.png" alt="Yosh Ijodkor logo" className="w-40 h-40 object-contain shrink-0" />
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black leading-[1.2] font-serif">
                {lang==='uz'
                  ? <>O'quvchilar ijodini<br/><span className="text-white">qo'llab-quvvatlang</span></>
                  : <>Поддержите творчество<br/><span className="text-white">школьников</span></>}
              </h1>
            </div>
            <p className="text-white/65 text-lg leading-relaxed mb-10 max-w-lg">{t('hero_subtitle')}</p>
            ...
            <div className="grid grid-cols-3 gap-6 pt-8 border-t border-white/10">
              {[
                {value:products.length+'+',label:t('hero_stats_products')},
                {value:'50+',label:t('hero_stats_students')},
                {value:'200+',label:t('hero_stats_sold')},
              ].map((s,i) => (
                <div key={i}>
                  <p className="text-3xl font-black text-[#ffffff] leading-none">{s.value}</p>
                  <p className="text-white/50 text-xs mt-1.5 font-semibold uppercase tracking-wide">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="hidden lg:grid grid-cols-2 gap-3">
            {products.slice(0,4).map((p,i) => (
              <div key={p.id}
                className={`rounded-2xl overflow-hidden border-2 border-[#ffffff]/20 shadow-2xl ${i===0?'col-span-2 h-52':'h-44'}`}>
                <img src={p.image} alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 60L1440 60L1440 20C1200 60 900 0 720 20C540 40 240 0 0 20L0 60Z" fill="#faf9ff"/>
          </svg>
        </div>
      </section>
      ...
      <section className="py-16 bg-[#faf9ff]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-10">
            <p className="text-2xl font-black text-[#4c1d95]">{t('cat_subtitle')}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {categories.map(cat => (
              <Link key={cat.id} to={`/products?cat=${cat.id}`}
                ...
                <div className={`w-20 h-20 shrink-0 rounded-2xl bg-gradient-to-br ${cat.color} flex items-center justify-center text-4xl shadow-xl group-hover:scale-110 transition-transform duration-300`}>
                  {cat.icon}
                </div>
                <div>
                  ...
                  <span className="inline-block mt-1.5 text-xs font-semibold text-[#4c1d95]/40 group-hover:text-white/60 transition-colors bg-[#4c1d95]/5 group-hover:bg-white/10 px-2.5 py-0.5 rounded-full">
                    {products.filter(p=>p.category===cat.id).length} ta mahsulot
                  </span>
                </div>
              </Link>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-3xl font-black text-[#4c1d95]">{t('prod_title')}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {newProducts.map((p,i) => (
              <div key={p.id} className="animate-fadeInUp" style={{animationDelay:`${i*80}ms`}}>
          ...
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-3xl font-black text-[#4c1d95]">
                {lang==='uz'?"Barcha ijodiy ishlar":"Все творческие работы"}
              </h2>
            </div>
            <Link to="/products"
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((f,i) => (
              <div key={i} className="group bg-[#faf9ff] hover:bg-[#4c1d95] rounded-2xl p-6 text-center transition-all duration-300 hover:shadow-xl hover:shadow-[#4c1d95]/25 hover:-translate-y-1 border border-[#4c1d95]/6">
                ...
                <h3 className="font-black text-[#4c1d95] group-hover:text-white mb-2 transition-colors">{lang==='uz'?f.uz:f.ru}</h3>
                <p className="text-gray-500 group-hover:text-white/60 text-sm leading-relaxed transition-colors">{lang==='uz'?f.duz:f.dru}</p>
              </div>
            ))}
          </div>
        </div>
