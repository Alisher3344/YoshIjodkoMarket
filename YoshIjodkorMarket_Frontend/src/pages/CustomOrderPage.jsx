...
import {
  ArrowLeft, CheckCircle, Upload, X, Palette, Scissors,
  PenTool, Package, HelpCircle, Clock, MapPin, Phone,
  DollarSign, FileImage, Send, Star
} from 'lucide-react'
import { useLangStore, useCustomOrderStore } from '../store'
...
export default function CustomOrderPage() {
  ...
  const types = [
    { id: 'painting', icon: '🎨', uz: 'Rasm / Portret', ru: 'Рисунок / Портрет' },
    ...
    { id: 'souvenir', icon: '🎁', uz: "Sovg'a / Suvenir", ru: 'Подарок / Сувенир' },
  ...
  const payments = [
    { id: 'partial', icon: '🟢', uz: "30% avval, 70% tayyor bo'lganda", ru: '30% сейчас, 70% после готовности' },
    { id: 'full',    icon: '🔵', uz: "To'liq oldindan to'lov (-10% chegirma)", ru: 'Полная предоплата (-10% скидка)' },
  ]

  const handleFile = (e) => {
    const file = e.target.files[0]
    if (!file) return
    if (file.size > 10 * 1024 * 1024) { toast.error(uz("Fayl 10MB dan katta!", "Файл больше 10МБ!")); return }
  ...
  const nextStep = () => {
    if (step === 1 && (!form.description || !form.type)) {
      toast.error(uz("Iltimos, barcha maydonlarni to'ldiring", "Пожалуйста, заполните все поля")); return
    }
    if (step === 2 && !form.budget) {
      toast.error(uz("Byudjetni tanlang", "Выберите бюджет")); return
    }
    setStep(s => s + 1)
  }
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.phone) {
      toast.error(uz("Ism va telefon majburiy!", "Имя и телефон обязательны!")); return
    }
    const id = addCustomOrder(form)
    setOrderId(id)
    setSubmitted(true)
      ...
      <div className="min-h-screen bg-[#faf9ff] pt-20 flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl p-10 max-w-md w-full shadow-xl border border-[#4c1d95]/10 text-center">
          ...
          <div className="bg-[#ede9fe] rounded-2xl px-5 py-3 mb-8">
            <p className="text-xs text-[#4c1d95]/60 mb-1">{uz("Buyurtma ID", "ID заказа")}</p>
    ...
    <div className="min-h-screen bg-[#faf9ff] pt-20">
      ...
      <div className="bg-[#4c1d95] hero-pattern py-10 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white/60 hover:text-white text-sm font-semibold mb-6 transition-colors">
            ...
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-black transition-all ${
                  ...
                  step === s ? 'bg-white text-[#4c1d95]' :
                  'bg-white/20 text-white/60'
                }`}>
                  {step > s ? '✓' : s}
                </div>
                <span className={`text-xs font-bold hidden sm:block ${step === s ? 'text-white' : 'text-white/50'}`}>
                  {s === 1 ? uz("Tavsif", "Описание") : s === 2 ? uz("Byudjet", "Бюджет") : uz("Ma'lumotlar", "Контакты")}
                </span>
                {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 w-8 sm:w-16 ${step > s ? 'bg-green-400' : 'bg-white/20'}`} />}
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-6 animate-fadeInUp">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#4c1d95]/8">
              ...
              <p className="text-gray-400 text-sm mb-4">
                {uz("Imkon qadar batafsil yozing", "Опишите как можно подробнее")}
              </p>
              <textarea
                ...
                rows={4}
                placeholder={uz(
                  'Masalan: "Qizim uchun qo\'lda tikilgan ko\'k rangli sumka, ustiga gul naqshi bilan, o\'lcham 25x30 sm"',
                  'Например: "Сумка ручной работы для дочери, голубого цвета с цветочным узором, размер 25x30 см"'
                )}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#4c1d95]/8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {types.map(tp => (
                  <button key={tp.id} onClick={() => setForm(f => ({ ...f, type: tp.id }))}
                    ...
                    <span className={`text-sm font-bold ${form.type === tp.id ? 'text-[#4c1d95]' : 'text-gray-600'}`}>
                      {lang === 'uz' ? tp.uz : tp.ru}
                    </span>
                  </button>
                ))}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#4c1d95]/8">
              <h3 className="font-black text-[#4c1d95] text-lg mb-1 font-serif flex items-center gap-2">
                ...
                {uz("Namuna yoki rasm yuklash", "Загрузить образец или фото")}
                <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                  {uz("ixtiyoriy", "необязательно")}
                </span>
              </h3>
              <p className="text-gray-400 text-sm mb-4">
                {uz("Xohlagan narsaga o'xshash rasm yuklang", "Загрузите похожее изображение для примера")}
              </p>
              {preview ? (
                <div className="relative inline-block">
                  <img src={preview} alt="preview" className="h-40 rounded-xl object-cover border-2 border-[#4c1d95]/20" />
                  <button onClick={() => { setPreview(null); setForm(f => ({ ...f, file: null })) }}
                    className="absolute -top-2 -right-2 w-7 h-7 bg-red-500 text-white rounded-full flex items-center justify-center">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
                <label className="flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed border-[#4c1d95]/20 rounded-xl cursor-pointer hover:bg-[#faf9ff] hover:border-[#4c1d95]/40 transition-all">
                  <Upload className="w-8 h-8 text-[#4c1d95]/40" />
                  <div className="text-center">
                    <p className="text-sm font-bold text-[#4c1d95]">{uz("Fayl tanlash", "Выбрать файл")}</p>
            ...
            <button onClick={nextStep}
              className="w-full bg-[#4c1d95] hover:bg-[#5b21b6] text-white font-black py-4 rounded-2xl transition-all hover:scale-[1.01] shadow-xl shadow-[#4c1d95]/25 text-base">
              {uz("Keyingisi →", "Далее →")}
            </button>
          </div>
        )}
          ...
          <div className="space-y-6 animate-fadeInUp">
              ...
              <div className="grid grid-cols-2 gap-3">
                {budgets.map(b => (
                  <button key={b.id} onClick={() => setForm(f => ({ ...f, budget: b.id }))}
                    ...
                    <p className={`font-black text-base ${form.budget === b.id ? 'text-[#4c1d95]' : 'text-gray-700'}`}>{b.label}</p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#4c1d95]/8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <button key={d.id} onClick={() => setForm(f => ({ ...f, deadline: d.id }))}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#4c1d95]/8">
              ...
              <div className="space-y-3">
                {payments.map(p => (
                  <button key={p.id} onClick={() => setForm(f => ({ ...f, payment: p.id }))}
                    ...
                    <span className={`text-sm font-bold ${form.payment === p.id ? 'text-[#4c1d95]' : 'text-gray-600'}`}>
                      {lang === 'uz' ? p.uz : p.ru}
                    </span>
                    {form.payment === p.id && (
          ...
          <div className="space-y-6 animate-fadeInUp">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#4c1d95]/8">
              ...
              <div className="space-y-4">
                {[
                  { key:'name', icon:<PenTool className="w-4 h-4"/>, label:uz("Ism Familiya","Имя Фамилия"), placeholder:uz("Ism Familiya","Имя Фамилия"), type:'text' },
                  { key:'phone', icon:<Phone className="w-4 h-4"/>, label:uz("Telefon","Телефон"), placeholder:'+998 90 000 00 00', type:'tel' },
                  { key:'address', icon:<MapPin className="w-4 h-4"/>, label:uz("Manzil (ixtiyoriy)","Адрес (необязательно)"), placeholder:uz("Shahar, ko'cha","Город, улица"), type:'text' },
                ].map(field => (
                  <div key={field.key}>
            ...
            <div className="bg-[#ede9fe] rounded-2xl p-5 border border-[#4c1d95]/15">
              <h4 className="font-black text-[#4c1d95] mb-3">{uz("Buyurtma xulosasi", "Итог заказа")}</h4>
              <div className="space-y-2 text-sm">
                {form.type && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">{uz("Turi","Тип")}:</span>
                    <span className="font-bold text-[#4c1d95]">{types.find(t=>t.id===form.type)?.[lang==='uz'?'uz':'ru']}</span>
                  </div>
                )}
                {form.budget && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">{uz("Byudjet","Бюджет")}:</span>
                    <span className="font-bold text-[#4c1d95]">{budgets.find(b=>b.id===form.budget)?.label} so'm</span>
                  </div>
                )}
                {form.deadline && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">{uz("Muddat","Срок")}:</span>
                    <span className="font-bold text-[#4c1d95]">
                      {[{id:'3d',uz:'3 kun',ru:'3 дня'},{id:'1w',uz:'1 hafta',ru:'1 неделя'},{id:'2w',uz:'2 hafta',ru:'2 недели'},{id:'1m',uz:'1 oy',ru:'1 месяц'}].find(d=>d.id===form.deadline)?.[lang]}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-500">{uz("To'lov","Оплата")}:</span>
                  <span className="font-bold text-[#4c1d95]">{form.payment==='partial'?uz("30% + 70%","30% + 70%"):uz("To'liq oldindan","Полная предоплата")}</span>
                </div>
              </div>
            </div>
